...
tempfile="/home/dyap/dyap_temp/assoc-factor4CG_input.temp"
...
sel=$dir"/assoc-factors_seq.txt"
sltd=$dir"/assoc-factors_seq_formatted.txt"
...
awk 'NR%2{printf $0" ";next;}1' $sel | awk -F"|" '{print ","$6"_"$1"_"$2"_"$9","$11}' | tr -d "> " | sed 's/CDS://' > $sltd
...
for i in `cat $sltd |  awk -F, '{print $2}' | sort -u ` 
	...
	if [[ $i == "___" ]];
		...
		cds_st=`grep -m$j $i $sltd | tail -n1 | awk -F, '{print $2}' | awk -F"_" '{print $4}' | awk -F- '{print $1}'` 
		cds_end=`grep -m$j $i $sltd | tail -n1 | awk -F, '{print $2}' | awk -F"_" '{print $4}' | awk -F- '{print $2}'` 
		seqlen=`echo $seq | wc -c` 

		cxt=`echo $seq | cut -c "$cds_st"-"$cds_end"`
		len=`echo $cxt | wc -c`
		win=`echo "$len - 1" | bc`
