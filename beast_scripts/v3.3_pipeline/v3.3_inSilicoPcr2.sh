...
# Version 2 modified to take custom primer set ID
# Instead of using Chr and SNV, we use start and end on custom amplicon
...
#	chr=`echo $id | awk -F"_" '{print $(NF-1)}'`
        cpos1=`grep -A3 $i $wkfile | grep P3_COMMENT2= | awk -F= '{print $2}' | awk -F"-" '{print $1}'`
        cpos2=`grep -A3 $i $wkfile | grep P3_COMMENT2= | awk -F= '{print $2}' | awk -F"-" '{print $2}'`
		...
		echo $id","$cpos1","$cpos2","$left","$right","$size >> $primerlist
		done
