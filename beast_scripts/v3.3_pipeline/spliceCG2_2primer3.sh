...
outfile=$outdir"/hct116_CG2_primer3_output.txt"
infile=$indir"/hct116_CG2_primer3_input.txt"
highden="/home/dyap/Projects/Takeda_T3/CG/HCT116_hTERT_CG_high_motif_dens.tsv"
allCG="/home/dyap/Projects/Takeda_T3/CG/hct116_htert_primer_pipeline_input.csv"
...
# If you want to use the all the results comment out this section
        ...
        tfas=$temp"/SpliceCG2_primer3_input.fa"

rm -f $tfas

#<<<-------SELECTED SVs--------->>>
# This is the file has we have used to short-list the ones that we want to work on
sel="/home/dyap/Projects/PrimerDesign/manual/hct116_CG2_primer_pipeline_input.csv"
sltd="/home/dyap/Projects/PrimerDesign/manual/hct116_CG2_primer_pipeline_input.txt"

rm -f $sel

for i in `cat $highden | awk -F"\t" '{print $3}' | sort -u` 
	do
	echo $i
	grep "$i" $allCG >> $sel
	grep "$i" $allCG | awk -F, '{print $2}' | sort -u
	echo "========================"
	done
...
cat $sel | awk -F, '{print "hct116-CG2,"$2","$3","$4}' | sort -u > $sltd
