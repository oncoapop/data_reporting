...
# This has been changed to accommodate the different format for splice joins which use : as separator
	ampchr=`grep -m1 $i $inputfile | awk -F">" '{print $2}' | awk -F":" '{print $1":"$2}' | sed 's/chr//'`
...
# This has been changed to take into consideration the name which uses : as a separator
        ...
        if [[ $plusstrand > 0 ]] && [[ $minusstrand == 0 ]];
                then
                ampstart=`grep -m1 $i $inputfile | awk -F":" '{print $3}' | awk -F"+" '{print $1}'`
	...
	ampchr=`grep -m1 $i $inputfile | awk -F">" '{print $2}' | awk -F":" '{print $1":"$2}' | sed 's/chr//'`
        ...
        ltpos=`grep -m1 $i $inputfile | awk -F" " '{print $2}' | awk -F_ '{print $2}' | awk -F- '{print $1}'`
        rtpos=`grep -m1 $i $inputfile | awk -F" " '{print $2}' | awk -F_ '{print $2}' | awk -F- '{print $2}'`
        pos=`echo "($ltpos + $rtpos) / 2" | bc`
...
# This is redundant as for this case, it chr is obtained from the same file and using same formular
	...
	echo "chr: "$chr",   ampchr:"$ampchr
	echo "llim: "$llim",   ampstart:"$ampstart
	echo "rlim: "$rlim",   ampend:"$ampend
	echo "reads: "$reads
	...
	if [[ $chr =~ $ampchr ]] && [[ $llim > $pos ]] && [[ $rlim < $pos ]];
		...
		Miseq="PASS"
			...
			ampchr=`grep -m$n $i $inputfile | tail -n1 | awk -F">" '{print $2}' | awk -F":" '{print $1":"$2}' | sed 's/chr//'`
