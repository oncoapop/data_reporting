...
tempfile="/home/dyap/dyap_temp/factor4CG_input.temp"
...
sel=$dir"/RBP_CG-factors_seq.txt"
sltd=$dir"/RBP_CG-factors_seq_formatted.txt"
...
#format: of $sel
#gene_name	gene_id	transcript_id	refseq_transcript_id	protein_id	domain	protein_start	protein_end	chrom	strand	starts	ends	sequence
#LIN28A	ENSG00000131914	ENST00000254231		ENSP00000254231	CSD	40	112	1	1	26737383,26737877,26751794,26752733	26737413,26738073,26751978,26752838	
#ATGGGCTCCGTGTCCAACC
...
cat $sel | tail -n +2 | awk -F"\t" '{print ","$1"_"$4"_"$5"_"$6","$13}' | tr -d " " > $sltd
...
# The window length does not give enough design space then only use first junction.
	...
	if [[ $len -gt $cutoff ]]
		...
		echo "SEQUENCE_TEMPLATE="$cxt ;
		echo "COMMENT="$leftdist","$len","$right;
		echo "P3_COMMENT="$cxt;
