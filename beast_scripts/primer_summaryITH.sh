...
infile=$dir"/positions/"$name"_pos.txt"
...
# primer file PAT10_chr18_19438541
# position file PAT10 18:19438541-19438541

		for j in `cat $infile  | awk -F- '{print $1}' | sed 's/\t/_chr/'  | sed 's/\:/_/'` 
		do
		echo .

# This tests to see if there are any primers returned, if not, the record is skipped
		test=`grep -A21 $j $sourcedir$fname | grep "PRIMER_PAIR_EXPLAIN="` 
...
# form at PAT10   18:19438541-19438541
readfile=$anodir"/"$name"_anno.txt"

                for k in `cat $infile  | awk -F, '{print "Chr"$2"_"}' | tr "c" "C" | tr -d '"' `
			...
			grep $k $readfile
		        ...
		        match=`grep $k $outfile"66.tmp" | awk -F" " '{print $2}'`
	               ...
	               test=`grep -A21 $k $sourcedir$fname | grep "PRIMER_PAIR_EXPLAIN="`
