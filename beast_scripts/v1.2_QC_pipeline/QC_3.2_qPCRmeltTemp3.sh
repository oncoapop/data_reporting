...
platedir="/share/lustre/backup/dyap/Projects/"$name"/qPCR"
...
# Only one plate and that plate is plate 2
###########################################################
...
do
	...
	grep "$m" $platedir"/"$name"-QC" > $pcrfile
	...
	for k in A B C D E F G H 
			...
			actemp=`grep -w $well $pcrfile | awk -F"\t" '{print $4}'` 
			CT=`grep -w $well $pcrfile | awk -F"\t" '{print $9}'` 

			# Get the name and calculated Tm and length from $Calculated (Cal_Temp output)
			ampname=`cat $Calculated | grep -m1 $primerplate1 | awk -F"\t" '{print $1}' | awk -F_  '{print $1"_"$2"_"$3}'`
			caltemp=`cat $Calculated | grep -m1 $primerplate1 | awk -F_  '{print $4}' |  sed 's/Tm=//'`
			ampleng=`cat $Calculated | grep -m1 $primerplate1 | awk -F_  '{print $5}' |  sed 's/Length=//'`
...
if [[ $secondplate == "TRUE" ]]
	...
	for q in A B C D E F G H 
			...
			# Get the name and calculated Tm and length from $Calculated (Cal_Temp output)
			ampname=`cat $Calculated | grep -m1 $primerplate2 | awk -F"\t" '{print $1}' | awk -F_  '{print $1"_"$2"_"$3}'`
			caltemp=`cat $Calculated | grep -m1 $primerplate2 | awk -F_  '{print $4}' |  sed 's/Tm=//'`
			ampleng=`cat $Calculated | grep -m1 $primerplate2 | awk -F_  '{print $5}' |  sed 's/Length=//'`
