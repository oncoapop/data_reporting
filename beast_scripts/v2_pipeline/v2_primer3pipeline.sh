...
for i in `cat $uniq`
	...
	echo "SEQUENCE_TARGET="$sta","$width >> $infile
        echo "PRIMER_PRODUCT_OPT_SIZE="$Miseq >> $infile
...
cd $outdir
primer3="/meta/o/oncoapop/Apps/primer3-2.3.5/src/primer3_core"
...
~/Scripts/v2_pipeline/v2_p3out2isPCRin.sh

echo "####################################################"
echo "Checking amplicons/primers using in-silico PCR"

#~/Scripts/v2_pipeline/v2_primer-check-isPCR.sh
Rscript ~/Scripts/v2_pipeline/v2_check_UCSC.R --no-save --no-restore --args $Project%$sample%$name%$Miseq 
#cp nohup.out $Project"_output.log"
