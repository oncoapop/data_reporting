...
for i in `cat $pri`
	...
	echo "Primers="$match >> $output2

	for j in $samples
		do
		match2=`grep $gene1 $input | grep $gene2 | grep $j | awk -F" " '{print $1" "$8" "$9" "$10}'`
		match3=`grep $gene1 $input | grep $gene2 | grep $j`

		if [[ $match2 == "" ]]
			then
			echo $j" N/A N/A N/A" >> $output
			echo $j" N/A N/A N/A"
			else
			echo $match2 >> $output
			echo $match3 >> $output2
