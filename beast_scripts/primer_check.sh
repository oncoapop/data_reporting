...
echo "Amplicon,Amplicon_Length,Amplicon_Tm" > $outfile"33.tmp"
					...
					echo "<H3>Right(3'->5')	= " $right"</H3><p>" >> $outfile"4.tmp"
				       ...
				       echo $left$invseq$right >> $outfile"22.tmp" 
                                       amplicon=`echo $left$invseq$right | wc -c`
                                                ...
                                                echo "Amplicon len= "$amplicon"         Amplicon Tm ˚C= "$tm >> $outfile"22.tmp"
                                                echo $left$invseq$right","$amplicon","$tm >> $outfile"33.tmp"	
					...
					invseq1=`grep -A2 $k $outfile"3.tmp" | grep -P -o '(?<='$left1')[A-Z]*(?='$right1')'`
						...
						if [[ $left1 != "" && $right != "" ]];
							then    {
								...
								amplicon=`echo $left1$invseq1$right1 | wc -c`

                                                                A=`echo $left1$invseq1$right1 | grep "A" -o | wc -l`
                                                                T=`echo $left1$invseq1$right1 | grep "T" -o | wc -l`
                                                                C=`echo $left1$invseq1$right1 | grep "C" -o | wc -l`
                                                                G=`echo $left1$invseq1$right1 | grep "G" -o | wc -l`
                                                        tm=`echo "(( 64.9 + (41 * ($C+$G)-(16.4))/($A+$T+$C+$G) ))" | bc`
                                                echo "Amplicon len= "$amplicon"         Amplicon Tm ˚C= "$tm >> $outfile"22.tmp"
                                		echo $left1$invseq1$right1","$amplicon","$tm >> $outfile"33.tmp" 
								}
						fi
...
cp $outfile"33.tmp" $outfile"_amplicons.csv"
