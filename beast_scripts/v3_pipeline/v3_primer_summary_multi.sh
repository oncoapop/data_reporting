...
for i in `cat $raw  | awk -F"," '{print $4}' | tr -d '"' | sort -u`
	...
	echo "i="$i
		...
		if [ $test -gt 5 ]
			then
			echo "Possible non-specific amplicon - not useful if run in multi mode"
			grep $i $inputfile
			note="Many amplicons"
			else
			note=" "
	...
	# This module is specific for command-line isPcr output
		...
		do
		ampchr=`grep -m$j $i $inputfile | tail -n1 | awk -F">" '{print $2}' | awk -F":" '{print $1}' | sed 's/chr//'`


	        plusstrand=`grep -m$j $i $inputfile | tail -n1 | grep [0-9]+[0-9] | wc -c`
        	minusstrand=`grep -m$j $i $inputfile | tail -n1 | grep [0-9]-[0-9] | wc -c`

        	if [[ $plusstrand > 0 ]] && [[ $minusstrand == 0 ]];
                	then
                	ampstart=`grep -m$j $i $inputfile | tail -n1 | awk -F":" '{print $2}' | awk -F"+" '{print $1}'`
                	ampend=`grep -m$j $i $inputfile | tail -n1 | awk -F"+" '{print $2}' | awk -F" " '{print $1}'`
        	fi
        	...
        	myid=`grep -m$j $i $inputfile | tail -n1 | awk -F" " '{print $2}'`
        	amplen=`grep -m$j $i $inputfile | tail -n1 | awk -F" " '{print $3}' | sed 's/bp//'`
        	leftpri=`grep -m$j $i $inputfile | tail -n1 | awk -F" " '{print $4}'`
        	rightpri=`grep -m$j $i $inputfile | tail -n1 | awk -F" " '{print $5}'`

        	chr=`echo $myid | awk -F"_" '{print $(NF-1)}' | sed 's/chr//'`
        	pos=`echo $myid | awk -F"_" '{print $NF}'`


		ann=`grep -m$j $pos $annofile3 | tail -n1 | awk -F"\t" '{print $1}'`
		gene=`grep -m$j $pos $annofile3 | tail -n1 | awk -F"\t" '{print $2}'`
		snv=`grep -m$j $pos $annofile3 | tail -n1 | awk -F" " '{print $6}'`

		snvchk=`grep -m$j $i $designfile | tail -n1 | awk -F"," '{print $6}' | tr -d '"'`
		idchk=`grep -m$j $i $designfile | tail -n1 | awk -F"," '{print $2}' | tr -d '"'`
		cxt=`grep -m$j $i $designfile | tail -n1 | awk -F"," '{print $7}' | tr -d '"' | tr "A-Z" "a-z"`

		lines=`echo "($amplen/51)+1" | bc`
        	ampseq=`grep -m$j -A$lines $i $inputfile  | tail -n$lines | sed 's/^>.*$//' | tr -d "\n"  | grep --color=always $cxt`
