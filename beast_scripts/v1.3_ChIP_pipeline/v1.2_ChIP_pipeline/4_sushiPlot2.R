...
ctrltreat<-paste(paste("ls",paste("grep",drugconc[m],sep=" "),sep="|"),"| grep IgG | grep 
treat_pileup.bedgraph")
ctrlcontrol<-paste(paste("ls",paste("grep",drugconc[m],sep=" "),sep="|"),"| grep IgG | grep 
control_lambda.bedgraph")

expttreat<-paste(paste("ls",paste("grep",drugconc[m],sep=" "),sep="|"),"| grep 0vs10 | grep 
treat_pileup.bedgraph")
exptcontrol<-paste(paste("ls",paste("grep",drugconc[m],sep=" "),sep="|"),"| grep 0vs10 | grep 
control_lambda.bedgraph")
