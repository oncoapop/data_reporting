...
# v4 using Steidl lab test pipeline

# primer3output in temp directory
# p3outfile=$tmp"/"$name"_primer3_output.txt"
# name is pulled from SEQUENCE_ID= (not PRIMER_SEQUENCE_ID=)
...
for i in `cat $p3outfile | grep "SEQUENCE_ID="`
	...
	chr=`echo $i  | awk -F= '{print $2}' | awk -F_ '{print $2}' | tr -d "chr" | tr -d "Chr"`
...
~/Scripts/v4.0_pipeline/v4.0_inSilicoPcr.sh
...
Rscript ~/Scripts/v4.0_pipeline/v4.0_GetSeq.R --no-save --no-restore --args $Project/$name/$file
...
~/Scripts/v4.0_pipeline/v4.0_primer_summary.sh
