...
echo "Running Primer_Summary version v4.0..."
...
for i in `cat $raw  | awk -F"," '{print $4}' | tr -d '"' | sort -u`
	...
	ampchr=`grep -m1 $i $inputfile | awk -F">" '{print $2}' | awk -F":" '{print $1}' | sed 's/chr//'`
        ...
        if [[ $plusstrand > 0 ]] && [[ $minusstrand == 0 ]];
                ...
                ampend=`grep -m1 $i $inputfile |  awk -F":" '{print $2}' | awk -F"+" '{print $2}'`
        fi
...
# get context and annotations from p3outfile
	cxt=`grep -m1 $i $p3outfile | grep P3_COMMENT | awk -F= '{print $2}' | awk -F: '{print $1}'` 
	afn=`grep -m1 $i $p3outfile | grep P3_COMMENT | awk -F= '{print $2}' | awk '{print $2":"$3":"$4}'`
