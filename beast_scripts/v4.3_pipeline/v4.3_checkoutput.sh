...
for i in `cat $raw  | awk -F"," '{print $4}' | tr -d '"' | sort -u`
	...
	maskseq=`grep $i $designfile | awk -F, '{print $8}' | grep --color=always $cxt | grep --color=always N |  GREP_COLOR="1;32" grep --color=always $leftpri | GREP_COLOR="1;32" grep --color=always $rightprir`
...
echo "+++++++++++++++++++++++++++++++"
	...
	if [[ $Miseq =~ "PASS" ]]
		...
		echo "^^^^^^^^^^^^^^^^ Amplicon only --------------------------------SNP/SNV/masked sequence vvvvvvvvvvvvvvvvvvvvvvvvvvvv _____________________________________________________" >> $htmlfile"1.tmp"
		grep $i $designfile | awk -F, '{print $8}' | grep --color=always $cxt | grep --color=always N |  grep --color=always $leftpri | grep --color=always $rightprir >> $htmfile"1.tmp"
