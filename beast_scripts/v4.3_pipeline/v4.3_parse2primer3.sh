...
for s in $samples
	...
	infile=$outdir"/"$s"_p3_design.txt"

	rm -f $outfile
	rm -f $p3outfile
	rm -f $p3outfile.cat

	for i in `cat $infile | awk -F"," '{print $2}' | tr -d '"' | tail -n +2`
		do
	...
	####################################################
		...
		ID=`grep -m1 $i $infile | awk -F"," '{print $2}' | tr -d '"' `
		Ref=`grep -m1 $i $infile | awk -F"," '{print $6}' | tr -d '"'`
		Alt=`grep -m1 $i $infile | awk -F"," '{print $7}' | tr -d '"'`
		...
		Seq=`grep -m1 $i $infile | awk -F"," '{print $9}' |  tr -d '"' | tr -c 'ATCGatcg' 'N' `
#		Seq=`grep -m1 $i $infile | awk -F"," '{print $9}' |  tr -d '"' | tr -d '[' | tr -d ']' | tr -c 'ATCGatcg' 'N' `
#		Seq=`grep -m1 $i $infile | awk -F"\t" '{print $9}' | tr -d '"' | tr -d '['  | sed 's/\/.*\]//g'`
...
grep -B18 "PAIR_NUM_RETURNED=0" $p3outfile > $p3outfile".failed"
