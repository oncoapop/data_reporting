...
echo "Running Primer_Summary version v4.3..."
...
raw=$posdir"/"$name"_p3_positions.txt"
designfile=$p3dir"/"$name"_p3_design.txt"
p3file=$p3dir"/"$name"_primer3_output.txt"
...
if [ -f $manifestfile ]
	...
	# read ans
		...
		echo "Name,Chromosome,Amplicon Start,Amplicon End,Upstream Probe Length,Downstream Probe Length" >> $manfile
...
############ PASS FILE
	...
	date >> $htmfile"1.tmp"
	echo " " >> $htmfile"1.tmp"
	echo "The First row is the USCS Amplicon with the primers in Green and the SNV context in blue. The Pos of interest is the 3' most base in the blue sequence." >> $htmfile"1.tmp"
...
################################
# Extraction from isPcr output #
###############################

for i in `cat $raw  | awk -F"," '{print $1}' | tr -d '"' | sort -u`
	do
	echo "i="$i
		# QC to test if grep is successful
		pdts=`grep $i $inputfile | wc -l`
		echo "This is the number of isPCR products returned: "$pdts
		if [ $pdts -eq 0 ]
			then
			echo "Pattern not found"
			note="No isPCR products"
		...
		if [ $pdts -gt 5 ]
			...
			grep $i $inputfile
			note="5 primers pairs - "$pdts" amplicons"
			else
			note=" "
		fi
	...
	ampchr=`grep -m1 $i $inputfile | awk -F">" '{print $2}' | awk -F":" '{print $1}' | sed 's/chr//'`
        ...
        if [[ $plusstrand > 0 ]] && [[ $minusstrand == 0 ]];
                ...
                ampend=`grep -m1 $i $inputfile |  awk -F":" '{print $2}' | awk -F"+" '{print $2}' | awk -F" " '{print $1}'`
        fi
	...
	# Parses the annotation files which have the ref and alt alleles carried through from version 4.1 onwards
	ann=`grep -m1 $pos $annofile3 | awk -F"\t" '{print $1}'`
	gene=`grep -m1 $pos $annofile3 | awk -F"\t" '{print $2}' | sed -e 's/([^()]*)//g' | tr "," "-"`
	snv=`grep -m1 $pos $annofile3 | awk -F" " '{print $6}'`
	ref=`grep -m1 $pos $annofile3 | awk -F" " '{print $6}'`
	alt=`grep -m1 $pos $annofile3 | awk -F" " '{print $7}'`
	rem=`grep -m1 $pos $annofile3 | awk -F" " '{print $8}'`
...
#        		fi
         		...
         		if [[ $plusstrand > 0 ]]  && [[ $minusstrand == 0 ]];
        	        	...
        	        	ampend=`grep -m$n $i $inputfile |  tail -n1 | awk -F":" '{print $2}' | awk -F"+" '{print $2}' | awk -F" " '{print $1}'`
...
#	if [[ $chr == $ampchr ]]; then echo "chr match"; fi
#	if [[ $llim > $pos ]]; then echo "within left limits"; fi
#	if [[ $rlim < $pos ]]; then echo "within right limits"; fi
	...
	if [[ $Miseq =~ "PASS" ]]
		...
		stat="Unknown"
		newid=`echo "SP_"$name":chr"$ampchr"_"$pos":REF_"$ref":ALT_"$alt":ST_"$stat":AN_"$ann"_"$gene":REM_"$rem`
		...
		rlen=`echo $rightpri | wc -c`
		rightlen=`echo "$rlen - 1" | bc`
		echo $newid,"chr"$ampchr,$ampstart,$ampend,$leftlen,$rightlen >> $manfile
...
echo " or online at http://godel.cluster.bccrc.ca/workflow/primer3check/v4_pipeline/"$Project"/"$sample

# Transfer the products to our internal webserver so that they may be browsed from the address given above
rsync -vr --progress $htmloutpath/$Project/$sample dyap@godel.cluster.bccrc.ca:/var/www/html/workflow/primer3check/v4_pipeline/$Project/
