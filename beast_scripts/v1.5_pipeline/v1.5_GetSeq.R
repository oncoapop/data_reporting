...
# Command line `Rscript v1.5_GetSeq.R --no-save --no-restore --args $dir/$sample/$file`
...
# source(file="~/Scripts/v1.5_pipeline/v1.5_GetSeq.R")
...
# commented lines are Done by v1.5_primerdesign.cgi
...
#  sequence <- snpdf[ri,5]
# For generation of the sequence from the REF seq
  sequence <- as.character(paste(getSeq(Hsapiens,chr,position1-seqoffset,position1),
              getSeq(Hsapiens,chr,position2+1,position2+seqoffset),
              sep=''))  
