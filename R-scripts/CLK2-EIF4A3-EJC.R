...
# cat  CLK2_interactors_GOList_V5.txt | grep "nonsense-mediated decay"  | awk -F"\t" '{print $2}' | awk -F";" '{print $2}'
# cat  CLK2_interactors_GOList_V5.txt | grep "exon-exon junction complex"  | awk -F"\t" '{print $2}' | awk -F";" '{print $2}'
system("grep \"nonsense-mediated decay\" CLK2_interactors_GOList_V5.txt | awk -F\"\t\" '{print $2}' | awk -F\";\" '{print $2}' > CLK2_NMD_V5.txt")
system("grep \"exon-exon junction complex\" CLK2_interactors_GOList_V5.txt | awk -F\"\t\" '{print $2}' | awk -F\";\" '{print $2}' > CLK2_EJC_V5.txt")

NMD<-read.table(file="CLK2_NMD_V5.txt", header=FALSE)
EJC<-read.table(file="CLK2_EJC_V5.txt", header=FALSE)
...
MS<-mutate(MS, NMD=ifelse(MS$Benjamini.Hochberg.adjusted.Wald.p.value.for.average.treatment.effect<0.05 & 
	MS$log2.average.treatment.effect.difference.from.control.>0 & MS$Gene.name %in% NMD$V1, "Nonsense Medicated Decay (n=6)", ""))
MS<-mutate(MS, EJC=ifelse(MS$Benjamini.Hochberg.adjusted.Wald.p.value.for.average.treatment.effect<0.05 & 
	MS$log2.average.treatment.effect.difference.from.control.>0 & MS$Gene.name %in% EJC$V1, "Exon Junction Complex (n=12)", ""))
...
highlight2=c("CLK2")
eejc=c("MAGOH","Y14","CWC22")

p = ggplot(MS, aes(log2.average.treatment.effect.difference.from.control., 
           -log10(Benjamini.Hochberg.adjusted.Wald.p.value.for.average.treatment.effect)), pch=20) +
  geom_point(aes(col=Legend)) +

  theme_light()+geom_text_repel(data = MS[(MS$Gene.name %in% EJC$V1 & !(MS$Gene.name %in% NMD$V1)),], aes(label=Gene.name),
  fontface = "bold", force = 7, nudge_y=c(10,10,8,3,12,20,-5), nudge_x=c(-0.1,-1.3,0.3,0.05,0.7,-2,0.05)) +
  theme_light()+geom_label_repel(data = MS[(MS$Gene.name %in% NMD$V1),], aes(label=Gene.name),
  fontface = "bold", force = 7, nudge_y=c(-5,15,17,-1,12,2), nudge_x=c(1,-1,-1,1.5,1,-2)) +
  theme_light()+geom_text_repel(data = MS[(MS$Gene.name %in% eejc),], aes(label=Gene.name),
  fontface = "bold", force = 7, nudge_y=c(-5,15), nudge_x=c(1,-1)) +
  ...
  annotate("text", x = 2.3, y = 84, label = "Exon-junction complex(EJC)/NMD", fontface = "bold") +
  annotate("label", x = -0.5, y = 84, label = "Nonsense-mediated decay(NMD)", fontface = "bold") +
  annotate("label", x = 3.7, y = 5.5, label = "Y14", fontface = "bold") +
  annotate("label", x = 2.48, y = -4, label = "BTZ/MLN51", fontface = "bold") +
...
pdf(file="SupplFig_CLK2_Interactors_EJC_NMD_eIF4A3.pdf", useDingbats=FALSE)
p
dev.off()
