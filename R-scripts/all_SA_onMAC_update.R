...
tnbc_add<-as.data.frame(unique(sort(tnbc_seq[!tnbc_seq$SA_ID %in% outdf$SAID,c("SA_ID")])))
...
#Join data sets
outdf3<-join(outdf, outdf2, type = "full")
...
# I assmume that everything that gets for seq gets an SA ID
# Therefore it makes sense that I only use the SA ID to probe the spreadsheets

for (ri in seq(rec)) {

		# First get all the SA ID and if SA ID is not there use Collaborator IDs

		test_said<-outdf3$SAID[ri]

                if ( nchar(test_said) > 1 )
				{
...
print (test_said) }
                ...
                # Using the Missing IDs as a guide for this data set
                outdf4$SAID[ri]<-as.character(missingIDs$SAID[ri])
