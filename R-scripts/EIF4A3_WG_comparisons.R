...
# On beast
wd="/share/lustre/backup/dyap/Projects/eIF4A3_NMD/SG_list"
pat="Mao_2016_Eif4a3"
setwd(wd)
...
# Mao et al mouse
MMu<-as.data.frame(read.csv(file_names[1],header=TRUE))
#MS<-read.csv(file_names[1],header=TRUE)
#results<-read.csv(file_names[1],header=TRUE)
colnames(MMu)[3]<-"logFC_Eif4a3_Emx1Cre"

# Highlight the SG from the Sg-65 list from Poul
SG1<-read.csv(file="Hu-Mu-list.csv", header=TRUE)

pat="Wang_eIF4A3_KD.csv"
setwd(wd)
file_names = list.files(pattern = pat)


WHu<-as.data.frame(read.csv(file_names[1],header=TRUE))
WHu$logFC_EIF4A3<-log10(WHu$FC_4A3)


# I am going to take another route that is to combine all the changes EJC genes


##############################################
###   Version for Meo at al plot  = V1     ###
###      Highlight SG factors in red       ###
##############################################

## PRELIM

#MaoMu=subset(MMu, FDR < 0.05 & logFC_Eif4a3_Emx1Cre > 0, select=c(GeneID,GeneName,IsCoding))
#WangHu=subset(WHu, padj4A3 < 0.05 & logFC_EIF4A3 > 0, select=c(Ensemble.ID,geneName))

MaoMu=subset(MMu, FDR < 0.05 & logFC_Eif4a3_Emx1Cre > 0,)
WangHu=subset(WHu, padj4A3 < 0.05 & logFC_EIF4A3 > 0,)

# Nice labelling (non-overlapping)
MS<-mutate(MS, Legend=ifelse(MS$PValue<0.05 & MS$logFC_Eif4a3_Emx1Cre>0, "p<0.05 (n=999)", "Not Significant (n=17465)"))
MS<-mutate(MS, SG=ifelse(MS$logFC_Eif4a3_Emx1Cre>0  & MS$GeneName %in% SG1$mouse, "Altered SG genes (36/63)", "Unknown"))
MS<-mutate(MS, SG3=ifelse(MS$PValue<0.05 & MS$logFC_Eif4a3_Emx1Cre>0  & MS$GeneName %in% SG1$mouse, "Significantly changed SG genes (5/63)", "Unknown"))

overlap1<-MS[MS$logFC_Eif4a3_Emx1Cre>0  & MS$GeneName %in% SG1$mouse,]
overlap1$GeneName
...
write.csv(allbinding, file="KD_Eif4a3.csv", quote=FALSE)

sgs<-as.list(as.character(allbinding$GeneName))

# Skip GO as too few genes
...
p = ggplot(MS, aes(logFC_Eif4a3_Emx1Cre, -log10(FDR)), pch=20) +
  geom_point(aes(col=Legend)) +
  theme_light()+
#  geom_label_repel(data = MS[(MS$SG == "Altered SG genes (36/63)"),], aes(label=GeneName)) +
  geom_label_repel(data = MS[(MS$SG3 == "Significantly changed SG genes (5/63)"),], aes(label=GeneName))
,  
  force = 7, nudge_y=c(20,-5,-5,5,5,23), nudge_x =c(-1.5,-0.15,-1.2,-1.2,-0.1,-1.23), color="black") +

  geom_point(aes(col=SG)) + 
  ...
  labs(x = "logFC_Eif4a3_Emx1Cre", y="-log10((FDR<0.05)") 
