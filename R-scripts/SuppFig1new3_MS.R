...
#	MS$log2.average.treatment.effect.difference.from.control.>1 & MS$Gene.name %in% Int$V1, "Known Interactors (34/97)", "Unknown"))
...
# Nice labelling (non-overlapping)
MS<-mutate(MS, Legend=ifelse(MS$Benjamini.Hochberg.adjusted.Wald.p.value.for.average.treatment.effect<0.05,
	"p<0.05 (n=707)", "Not Significant (n=520)"))
MS<-mutate(MS, Interactors=ifelse(MS$Benjamini.Hochberg.adjusted.Wald.p.value.for.average.treatment.effect<0.05 & 
...
MS<-mutate(MS, Enriched=ifelse(MS$Benjamini.Hochberg.adjusted.Wald.p.value.for.average.treatment.effect<0.05 
	& MS$Gene.name %in% CG.enrich$V1, "CG Enriched (n=12)", ""))
...
#  theme_light()+geom_text_repel(data = MS[MS$Both == "Both (n=19)",], aes(label=Gene.name), fontface = "bold", force = 7) +
  theme_light()+geom_text_repel(data = MS[MS$Enriched == "CG Enriched (n=12)",], aes(label=Gene.name), 
...
pdf(file="SupplFig_CLK2_Interactors_CG+enriched.pdf")
p
dev.off()
