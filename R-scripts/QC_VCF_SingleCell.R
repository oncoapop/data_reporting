...
basedir="/share/lustre/backup/dyap/Projects/Single_Cell"
sample="184-hTert"
run="3530550"
#check="depth"
check = "freq"
...
workdir=paste(basedir, sample, sep="/")
setwd(workdir)
...
samplesheet<-read.csv("Samplesheet.csv", skip=30)
...
title=paste(sample, "Single Cell Expt - all variants", sep=" ")

xaxislab2=paste("Samples from Run", run, sep=" ")

pdf(pdffile3, width=7, height=8)

heatmap.2(ef, main=title, xlab=xaxislab2, ylab="Positions", scale="none", key = TRUE
, cexCol=0.8, cexRow=0.6, col = hmcols, RowSideColors=rsc$Col, 
trace="none")
