...
pat="CLK_resMat_PSM_GeneLevel_vsn_Vars_v01.csv"
setwd(wd)
...
# Highlight known Uniprot/BioGrid/IntAct UBI database interactors of CLK1
Int<-read.table(file="all_UBIdb_interactors.txt", header=FALSE, sep="\t")

##############################################
###   Version for Enrichment lg2 = 0 - V1  ###
##############################################
## 2 x FLAG Tagged CLK1
## 2 x Untagged CLK1
## 2 x Empty vector control

## For untagged CLK1

untag=subset(MS, Untagged.N.Flagged.Benjamini.Hochberg.adjusted.Wald.p.value.for.average.treatment.effect < 0.05 & 
	Untagged.N.Flagged.log2.average.treatment.effect.difference.from.control.> 0, select=c(Accession,Gene.name,Descriptions))
	#,pepNum))


# Nice labelling (non-overlapping)
MS<-mutate(MS, Legend=ifelse(MS$Untagged.Benjamini.Hochberg.adjusted.Wald.p.value.for.average.treatment.effect<0.05 & 
	MS$Untagged.N.Flagged.log2.average.treatment.effect.difference.from.control.>0, "p<0.05 (n=327)", "Not Significant (n=424)"))
MS<-mutate(MS, Interactors=ifelse(MS$Untagged.Benjamini.Hochberg.adjusted.Wald.p.value.for.average.treatment.effect<0.05 & 
	MS$Untagged.N.Flagged.log2.average.treatment.effect.difference.from.control.>0 & MS$Gene.name %in% Int$V1, "Known Interactors (37/171)", 
"Unknown"))
MS<-mutate(MS, Interactors=ifelse(MS$Gene.name %in% Int$V1, "Known Interactors (37/171)", "Unknown"))
...
write.csv(untag, file="untagCLK1_interactors_MS_V1.csv", quote=FALSE)
clk1<-as.list(as.character(untag$Gene))
system("rm -f CLK1_int_genes_V1.txt")
lapply(clk1, write, "CLK1_int_genes_V1.txt", append=TRUE, ncolumns=nrow(untag))

# Submit this file to http://www.pantherdb.org/geneListAnalysis.do
# remove the "'" from 3' as it causes import failure
GO<-read.table(file="CLK1-bind_MS-GO_V1.txt", header=TRUE, skip=10, sep="\t")
...
Splice<-read.table(file="CLK1_inv_Splicing_V1.txt", header=FALSE)
CG.enrich<-read.table(file="/home/dyap/Projects/Takeda_T3/CG_factors/CG_RNA_motifs_full", header=FALSE)
EndPro <-read.table(file="CLK1_3endproc_V1.txt", header=FALSE)
...
MS<-mutate(MS, Legend=ifelse(MS$Untagged.Benjamini.Hochberg.adjusted.Wald.p.value.for.average.treatment.effect<0.05 & 
	MS$Untagged.N.Flagged.log2.average.treatment.effect.difference.from.control.>0, "p<0.05 (n=327)", "Not Significant (n=424)"))
MS<-mutate(MS, Interactors=ifelse(MS$Untagged.Benjamini.Hochberg.adjusted.Wald.p.value.for.average.treatment.effect<0.05 & 
	MS$Untagged.N.Flagged.log2.average.treatment.effect.difference.from.control.>0 & 
	MS$Gene.name %in% Int$V1, "Known Interactors (27/172)", ""))
MS<-mutate(MS, Function=ifelse(MS$Untagged.Benjamini.Hochberg.adjusted.Wald.p.value.for.average.treatment.effect<0.05 & 
	MS$Untagged.N.Flagged.log2.average.treatment.effect.difference.from.control.>0 & 
	MS$Gene.name %in% Splice$V1, "RNA splicing (n=87)", ""))
MS<-mutate(MS, Both=ifelse(MS$Untagged.Benjamini.Hochberg.adjusted.Wald.p.value.for.average.treatment.effect<0.05 & 
	MS$Untagged.N.Flagged.log2.average.treatment.effect.difference.from.control.>0 & 
	MS$Gene.name %in% Splice$V1 & MS$Gene.name %in% Int$V1, "Both (n=17)", ""))
MS<-mutate(MS, EndProc=ifelse(MS$Untagged.Benjamini.Hochberg.adjusted.Wald.p.value.for.average.treatment.effect<0.05 & 
	MS$Untagged.N.Flagged.log2.average.treatment.effect.difference.from.control.>0 & MS$Gene.name %in% EndPro$V1, "3'-end Processing (n=26)", ""))
MS<-mutate(MS, Enriched=ifelse(MS$Untagged.Benjamini.Hochberg.adjusted.Wald.p.value.for.average.treatment.effect<0.05 & 
	MS$Untagged.N.Flagged.log2.average.treatment.effect.difference.from.control.>0 & 
	MS$Gene.name %in% CG.enrich$V1, "CG Enriched (n=11)", ""))
...
p = ggplot(MS, aes(Untagged.log2.average.treatment.effect.difference.from.control., 
           -log10(Untagged.Benjamini.Hochberg.adjusted.Wald.p.value.for.average.treatment.effect)), pch=20) +
  geom_point(aes(col=Legend)) +
  theme_light()+
  geom_label_repel(data = MS[(MS$Enriched == "CG Enriched (n=11)" & MS$EndProc != "3'-end Processing (n=26)"),], aes(label=Gene.name),  
  fontface = "bold", force = 7, nudge_y=10, nudge_x =5, color="black") +
  geom_label_repel(data = MS[(MS$Enriched == "CG Enriched (n=11)" & MS$EndProc == "3'-end Processing (n=26)"),], aes(label=Gene.name),  
  fontface = "bold", force = 7, nudge_y=c(1,5,10), nudge_x =c(-3,-5,-6), color="red") +
  geom_text_repel(data = MS[(MS$Enriched != "CG Enriched (n=11)" & MS$EndProc == "3'-end Processing (n=26)"),], aes(label=Gene.name),  
  fontface = "bold", force = 7, nudge_y=60, nudge_x =2, color="red") +
  geom_text_repel(data = MS[(MS$Both ==  "Both (n=17)"  & MS$EndProc != "3'-end Processing (n=26)"),], aes(label=Gene.name),  
  fontface = "bold", force = 7, nudge_y=30, nudge_x =-2, color="orange") +

  geom_point(aes(col=Function)) + 
  geom_point(aes(col=Interactors)) + 
  geom_point(aes(col=EndProc)) + 
  geom_point(aes(col=Enriched)) + 
  scale_color_manual(values=c("transparent","red","blue", "orange","grey90","grey40","darkgreen")) +
  theme(legend.position = c(0.18, 0.75)) + ggtitle("CLK1 interactors by IP-MS") +
  theme(panel.border = element_rect(fill = NA, colour = "black", size = 1))+
  annotate("text", x = 4, y = 100, label = "Factors with motifs enriched in CG", fontface = "bold") +
...
pdf(file="SupplFig_CLK1_Interactors_invol_splice_3end.pdf")
p
dev.off()
...
# Download CLK1_interactors_GOList_Vn from PantherGO
system("grep \"splic\" CLK1_interactors_GOList_V1.txt | awk -F\"\t\" '{print $3}' | awk -F\";\" '{print $2}' > CLK1_inv_Splicing_V1.txt")

Splice<-read.table(file="CLK1_inv_Splicing_V1.txt", header=FALSE)
CG.enrich<-read.table(file="/home/dyap/Projects/Takeda_T3/CG_factors/CG_RNA_motifs_full", header=FALSE)

# Highlight known interactors of CLK1 (UBI databases)
...
# grep "RNA binding" CLK1_interactors_GOList_V1.txt | awk -F"\t" '{print $3}' | awk -F";" '{print $2}' > CLK1_int_mRNAbinding.txt
system("grep \"RNA binding\" CLK1_interactors_GOList_V1.txt | awk -F\"\t\" '{print $3}' | awk -F\";\" '{print $2}' > CLK1_inv_mRNAbinding_V1.txt")

RBin<-read.table(file="CLK1_inv_mRNAbinding_V1.txt", header=FALSE)

# grep "end-processing" CLK1_interactors_GOList_V1.txt | awk -F"\t" '{print $3}' | awk -F";" '{print $2}' > CLK1_3endproc_V1.txt
system("grep \"end-processing\" CLK1_interactors_GOList_V1.txt | awk -F\"\t\" '{print $3}' | awk -F\";\" '{print $2}' > CLK1_3endproc_V1.txt")
system("grep \"end processing\" CLK1_interactors_GOList_V1.txt | awk -F\"\t\" '{print $3}' | awk -F\";\" '{print $2}' >> CLK1_3endproc_V1.txt")
system("grep \"poly(A) binding\" CLK1_interactors_GOList_V1.txt | awk -F\"\t\" '{print $3}' | awk -F\";\" '{print $2}' >> CLK1_3endproc_V1.txt")
system("grep \"termination of RNA polymerase II transcription\" CLK1_interactors_GOList_V1.txt | awk -F\"\t\" '{print $3}' | awk -F\";\" '{print $2}' > CLK1_Pol2term_V1.txt")

EndPro <-read.table(file="CLK1_3endproc_V1.txt", header=FALSE)
Pol2Term<- read.table(file="CLK1_Pol2term_V1.txt", header=FALSE)

##########

MS<-mutate(MS, Legend=ifelse(MS$N.Flagged.Benjamini.Hochberg.adjusted.Wald.p.value.for.average.treatment.effect<0.05 & 
	MS$N.Flagged.log2.average.treatment.effect.difference.from.control.>0, "Above threshold (n=270)", "Below threshold (n=957)"))
MS<-mutate(MS, Interactors=ifelse(MS$N.Flagged.Benjamini.Hochberg.adjusted.Wald.p.value.for.average.treatment.effect<0.05 & 
	MS$N.Flagged.log2.average.treatment.effect.difference.from.control.>0 & 
	MS$Gene.name %in% Int$V1, "Known Interactors (28/97)", ""))
MS<-mutate(MS, Function=ifelse(MS$N.Flagged.Benjamini.Hochberg.adjusted.Wald.p.value.for.average.treatment.effect<0.05 & 
	MS$N.Flagged.log2.average.treatment.effect.difference.from.control.>0 & 
	MS$Gene.name %in% Splice$V1, "RNA splicing (n=98)", ""))
MS<-mutate(MS, Unpi=ifelse(MS$N.Flagged.Benjamini.Hochberg.adjusted.Wald.p.value.for.average.treatment.effect<0.05 & 
	MS$N.Flagged.log2.average.treatment.effect.difference.from.control.>0 & MS$Gene.name %in% Uniprot$V1, 
	"Uniprot Interactors (n=10/26)", ""))
MS<-mutate(MS, Bigi=ifelse(MS$N.Flagged.Benjamini.Hochberg.adjusted.Wald.p.value.for.average.treatment.effect<0.05 & 
	MS$N.Flagged.log2.average.treatment.effect.difference.from.control.>0 & MS$Gene.name %in% Biogrid$V1, 
	"Biogrid Interactors (n=27/69)", ""))
MS<-mutate(MS, Inai=ifelse(MS$N.Flagged.Benjamini.Hochberg.adjusted.Wald.p.value.for.average.treatment.effect<0.05 & 
	MS$N.Flagged.log2.average.treatment.effect.difference.from.control.>0 & MS$Gene.name %in% IntAct$V1, 
	"IntAct Interactors (n=13/52)", ""))
MS<-mutate(MS, Binders=ifelse(MS$N.Flagged.Benjamini.Hochberg.adjusted.Wald.p.value.for.average.treatment.effect<0.05 & 
	MS$N.Flagged.log2.average.treatment.effect.difference.from.control.>0 & MS$Gene.name %in% RBin$V1, "RNA binding (n=149)", ""))
MS<-mutate(MS, EndProc=ifelse(MS$N.Flagged.Benjamini.Hochberg.adjusted.Wald.p.value.for.average.treatment.effect<0.05 & 
	MS$N.Flagged.log2.average.treatment.effect.difference.from.control.>0 & MS$Gene.name %in% EndPro$V1, "3'-end Processing (n=31)", ""))
MS<-mutate(MS, Enriched=ifelse(MS$N.Flagged.Benjamini.Hochberg.adjusted.Wald.p.value.for.average.treatment.effect<0.05 & 
	MS$N.Flagged.log2.average.treatment.effect.difference.from.control.>0 & MS$Gene.name %in% CG.enrich$V1, "Motif density change in CG (n=10)", ""))
#MS<-mutate(MS, Transterm=ifelse(MS$N.Flagged.Benjamini.Hochberg.adjusted.Wald.p.value.for.average.treatment.effect<0.05 & 
#	MS$N.Flagged.log2.average.treatment.effect.difference.from.control.>0 & MS$Gene.name %in% Pol2Term$V1, "termination of RNA polymerase II transcription (n=xx)", ""))
MS<-mutate(MS, Z.both=ifelse(MS$N.Flagged.Benjamini.Hochberg.adjusted.Wald.p.value.for.average.treatment.effect<0.05 &
        MS$N.Flagged.log2.average.treatment.effect.difference.from.control.>0 & MS$Gene.name %in% CG.enrich$V1 & MS$Gene.name %in% EndPro$V1, "Motif change & 3'-end Processing (n=4)", ""))
...
suptab <- sigcom[c("Gene.name","Accession","Descriptions","Sequence","Annotated.Sequence","Modifications",
	...
	"N.Flagged.log2.average.treatment.effect.difference.from.control.","Average.treatment.effect.t.statistic.Wald.based.p.value",
...
p = ggplot(MS, aes(N.Flagged.log2.average.treatment.effect.difference.from.control., 
           -log10(N.Flagged.Benjamini.Hochberg.adjusted.Wald.p.value.for.average.treatment.effect)), pch=20) +
  geom_point(aes(col=Legend)) +
  theme_light()+
