...
##########################################
### Version for Enrichment lg2 = 0 - V4 ##
##########################################
...
write.csv(allbinding, file="CLK2_interactors_MS_V4.csv", quote=FALSE)
clk2<-as.list(as.character(allbinding$Gene))
system("rm -f CLK2_int_genes_V4.txt")
lapply(clk2, write, "CLK2_int_genes_V4.txt", append=TRUE, ncolumns=nrow(allbinding))
# Submit this file to http://www.pantherdb.org/geneListAnalysis.do
# remove the "'" from 3' as it causes import failure
GO<-read.table(file="CLK2-bind_MS-GO_new_V4.txt", header=TRUE, skip=10, sep="\t")
...
#  theme_light()+geom_text_repel(data = MS[MS$Both == "Both (n=19)",], aes(label=Gene.name), fontface = "bold", force = 7) +
  theme_light()+geom_text_repel(data = MS[MS$Enriched == "CG Enriched (n=10)",], aes(label=Gene.name), 
	fontface = "bold", force = 7) +
  geom_point(aes(col=Enriched)) + 
  scale_color_manual(values=c("transparent","dark green","grey90", "grey50")) +
  theme(legend.position = c(0.18, 0.85)) + ggtitle("CLK2 interactors by IP-MS (without enrichment cut-off)") +
...
sigcom$Enrich[sigcom$Enrich == "Motif denisty change in CG (n=10/47)"] <- "YES"
...
MS<-mutate(MS, Color=ifelse(MS$Benjamini.Hochberg.adjusted.Wald.p.value.for.average.treatment.effect<0.05 & 
	MS$log2.average.treatment.effect.difference.from.control.>0 & 
	MS$Gene.name %in% CG.enrich$V1, "darkdreen", "black"))
...
left=c("SRSF11","EIF4A3","SNRNP70","CLK1")
leftlab=c("SRSF1","SRSF6","MATR3","KHDRBS1")
right=c("DDX46","SRRM2","CPSF7","SRSF3","TRA2B","U2AF1","YTHDC1",
	"RNPS1", "DHX15","TNPO3","TRA2A","SON","NUDT21","LUC7L")
top=c("CLK2","THRAP3","SRRM1","ACIN1","SRSF7",
	"CLK4","LUC7L3","RBM39","LUC7L2","DDX5","CLK3","PNN")
toplab=c("SFPQ")
below=c("CPSF1","SRSF10","SMN1")
belowlab=c("U2AF2","ZNF638","HNRNPC","HNRNPH1","ELAVL1")
...
#  scale_color_manual(values=c("light grey", "dark grey")) +
  ...
  fontface = "bold", force = 7, nudge_y=-1.53, nudge_x = 1.26) +
  theme_light()+geom_label_repel(data = MS[MS$Gene.name %in% belowlab,], aes(label=Gene.name),
  fontface = "bold", force = 7, nudge_y=-1.53) +
...
pdf(file="SupplFig_CLK2_Interactors_Splicing_V5draft.pdf")
p
dev.off()
