...
availSNP <- available.SNPs()[len]
SNP <-   installed.SNPs()
# If availSNP is not currently installed load the latest version from source
#if (availSNP == SNP) print("Latest SNPS loaded") 
...
# We have to find the correct sample name, then file and path
...
#check <-  as.character(getSeq(Hsapiens,Chr,Pos-5,Pos+5))
  ...
  dseq <- 
paste(paste(as.character(getSeq(SNP_Hsapiens,Chr,Pos-offset,Pos-1)),"[",idseq,"/",Alt,"]",sep=""),
              as.character(getSeq(SNP_Hsapiens,Chr,Pos+1,Pos+offset)),sep="")

check <-  as.character(getSeq(SNP_Hsapiens,Chr,Pos-5,Pos+5))
